...
def main():
    ...
    config = DatacardConfigUnits(configFile, configparser)
    dc = Datacard(config)
    dc.produceDatacards()

class DatacardConfigUnits:
    # setup
	...
	with open('%s/%s' % (self.setupbase, self.binmapfile)) as json_file:
...
class Datacard:
	...
	for key in self.json_data:
		for subkey in self.json_data[key]:
			if "lepcr" == subkey:
				for subsubkey in self.json_data[key][subkey]:
					print("SubSubKey: " + subsubkey)
					self.srtounitbinmaps[subsubkey] = self.json_data[key][subkey][subsubkey]

	print(self.srtounitbinmaps)
