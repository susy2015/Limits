...
from ROOT import gROOT, gInterpreter, TFile, TTree, TH1D, TH2D, TChain, TGraph2D
gROOT.SetBatch(True)
...
def main():
    ...
    parser.add_argument("--covariance", dest="runCovariance", action='store_true',
                        help="Run mlfit for covariance matrix. [Default: False]")
    parser.add_argument("--mask-sr", dest="maskSignalRegions", action='store_true',
                        help="Masking SRs. [Default: False]")
...
#                         help="Name of output file with exclusion curves and interpolated cross section limits. [Default: results_T2tt.root]")
    ...
    parser.add_argument("-t", "--submittype", dest="submittype", default="interactive", choices=["interactive","condor"], 
                        help="Method of job submission. [Options: interactive, condor. Default: interactive]")
    parser.add_argument("--jobdir", dest="jobdir", default="jobs", help="Job dir. [Default: %(default)s]")
    parser.add_argument("--num-threads", dest="num_threads", type=int, default=-1, help="Number of threads to use. [Default: N(CPU)-2]")
    parser.add_argument("--runSL", dest="runSL", action='store_true', help="Run simplified likelihood.")
    parser.add_argument("--SL-input", dest="SL_input", default='SLinput.root', help="Input bkg/covar file for simplified likelihood.[Default: %(default)s]")
    parser.add_argument("--SL-signal-path", dest="SL_signal_path", default='signals', help="Input signal file directory for simplified likelihood.[Default: %(default)s]")
    args = parser.parse_args()
    if args.num_threads<0: 
        args.num_threads = multiprocessing.cpu_count()-2
    if args.runSL:
        scriptLH = '/'.join([os.environ['CMSSW_BASE'],'src/Limits/stats-tools/simplifiedLikelihood.C'])
        gInterpreter.ProcessLine('#include "%s"'%scriptLH)
    ...
    if args.printLimits:
        ...
        if limconfig.limitmethod == 'Asymptotic' or limconfig.limitmethod == 'AsymptoticObs':
            ...
            if args.runSL:
                runSimplifiedLimits(args, limconfig, limitFile, exclusionFile, args.addInterpolation)
            else:
                fillAsymptoticLimits(limconfig, limitFile, exclusionFile, args.addInterpolation)
    elif args.maxLikelihoodFit:
        runMaxLikelihoodFit(limconfig, args, args.signalPoint)
    else:
        runLimits(limconfig, args)
...
class LimitConfig:
  ...
  def __init__(self, conf_file, config_parser):
    ...
    self.binlist = ast.literal_eval(config_parser.get('bins', 'binlist'))
    self.binnumberdict = {self.binlist[ibin]:ibin for ibin in range(len(self.binlist))}
...
def fillSignificances(config, sigfile, name):
    ...
    minmlsp = 0.0
    mstop_step = 1
    mlsp_step = 10 if ('T2fbd' in sigfile or 'T2cc' in sigfile or 'T2bWL' in sigfile) else 1
    for signal in config.signals:
...
def fillAsymptoticLimits(config, limfilename, excfilename, interpolate):
    limits = []
    currentDir = os.getcwd()
    xsecfilename = os.path.join(os.environ['CMSSW_BASE'], 'src/data/xsecs/stop.root')
...
def runSimplifiedLimits(args, config, limfilename, excfilename, interpolate):
    from ROOT import simplifiedLikelihood
    ...
    for signal in config.signals:
        ...
        if minmstop == 0.0 or mstop < minmstop: minmstop = mstop
        if mlsp < minmlsp: minmlsp = mlsp
    if 'T2fbd' in limfilename or 'T2bWL' in limfilename or 'T2cc' in limfilename:
#         mlsp_step = 10
        minmlsp = 0
        maxmlsp = 85
    nbinsx = int((maxmstop - minmstop) / mstop_step)
    ...
    for signal in config.signals:
        sigfileSL = os.path.join(args.SL_signal_path, signal, signal+'.root')
        dummyDirSL = 'dummySL'
        if not os.path.exists(dummyDirSL):
            os.makedirs(dummyDirSL)
        outfileSL = os.path.join(dummyDirSL,signal)

        limit = {}
        limit['-1'] = 1e9
        limit['+1'] = 1e9
        limit['0'] = simplifiedLikelihood('signal', outfileSL, args.SL_input, sigfileSL, True)
        limit['obs'] = simplifiedLikelihood('signal', outfileSL, args.SL_input, sigfileSL, False)

        limits.append('%20s: exp: %.2f, obs: %.2f'%(signal, limit['0'],limit['obs']))
...
def runMaxLikelihoodFit(config, args, signal):
    ...
    dummyRunDir = os.path.join(args.jobdir, 'mlfit' ,signal)
    os.system("mkdir -p %s" % dummyRunDir)
    os.chdir(dummyRunDir)
    # from here on, work in this dummyRunDir

    combinedDatacardTXT = 'combined_' + signal + '.txt'
    combineDatacardsCommand = 'combineCards.py'
    for datacard in datacards:
        name = datacard.replace('.txt','')
        if name in config.binnumberdict:
            name = 'ch%03d'%config.binnumberdict[name]
        combineDatacardsCommand += ' ' + name + '=' + os.path.join(datacardSaveLocation, datacard)
    combineDatacardsCommand += ' > ' + combinedDatacardTXT
    lprint('', 'Combining datacards for %s'%signal)
    output = commands.getoutput(combineDatacardsCommand)

    convertDatacardCommandList = ['text2workspace.py']
#     convertDatacardCommandList.append('--optimize-simpdf-constraints=cms')
    if args.maskSignalRegions:
        convertDatacardCommandList.append('--channel-masks')
    convertDatacardCommandList.append(combinedDatacardTXT)
    convertDatacardCommand = ' '.join(convertDatacardCommandList)
    if args.submittype == 'interactive':
        lprint(convertDatacardCommand, '')
        commands.getoutput(convertDatacardCommand)
    combinedDatacard = combinedDatacardTXT.replace('.txt', '.root')

    # run the maximum likelihood fit
    runLimitsCommandList = ['combine', '-M MaxLikelihoodFit', '--saveNormalizations', '--saveShapes', '--saveWithUncertainties']
    if args.runCovariance:
        runLimitsCommandList += ['--numToysForShape 2000', '--saveOverallShapes']
    if args.maskSignalRegions:
        runLimitsCommandList += ['--setPhysicsModelParameters', ','.join(['mask_ch%03d=1'%ibin for ibin in range(len(config.binlist))])]
    runLimitsCommandList += [combinedDatacard]
    runLimitsCommand = ' '.join(runLimitsCommandList)
    if args.submittype == 'interactive':
        lprint(runLimitsCommand, 'running...')
...
def calcLimit(config, args, signal):
    ...
    # each signal loop, all remaining files will be either deleted or moved to
    # the actual output directory.
    dummyRunDir = os.path.join(args.jobdir, signal)
    os.system("mkdir -p %s" % dummyRunDir)
    os.chdir(dummyRunDir)
    # from here on, work in this dummyRunDir
    ...
    combineDatacardsCommand = 'combineCards.py'
    for datacard in datacards:
        combineDatacardsCommand += ' ' + datacard.replace('.txt','') + '=' + os.path.join(datacardSaveLocation, datacard)
    combineDatacardsCommand += ' > ' + combinedDatacardTXT
    lprint('', 'Combining datacards for %s'%signal)
    output = commands.getoutput(combineDatacardsCommand)

    convertDatacardCommand = 'text2workspace.py --optimize-simpdf-constraints=cms %s'%combinedDatacardTXT
    if args.submittype == 'interactive':
        commands.getoutput(convertDatacardCommand)
    combinedDatacard = combinedDatacardTXT.replace('.txt', '.root')
    ...
    if config.limitmethod == 'AsymptoticLimits':
        ...
        runLimitsCommand = 'combine -M Asymptotic -H ProfileLikelihood ' + combinedDatacard + ' -n ' + signal
        if (mstop<450 and 'fbd' not in sigtype) or (mstop >= 350 and mlsp < 350 and 'T2tt' in sigtype) :
            runLimitsCommand = 'combine -M Asymptotic -H ProfileLikelihood ' + combinedDatacard + ' --rMin 0 --rMax 10 -n ' + signal
        if ('fbd' in sigtype or '4bd' in sigtype) and (mstop<=250):
            runLimitsCommand = 'combine -M Asymptotic -H ProfileLikelihood ' + combinedDatacard + ' --rMin 0 --rMax 1 -n ' + signal
    ...
    # make output dir
    outputLocation = os.path.abspath(os.path.join(currentDir, config.limitdir, signal))
    ...
    if args.submittype == 'interactive':
        ...
        # still in this dummyRunDir, i.e., jobs/signal_point
        with open(os.path.join(thisScriptDir, 'runlimitscondor.sh')) as f:
            template_script = f.read()
        with open('runlimits.sh', 'w') as f:
            f.write(template_script.replace('__COMBINE_COMMAND__', '\n'.join([convertDatacardCommand, runLimitsCommand])))
        os.system('chmod +x runlimits.sh')
        with open('submit_%s.cmd' % signal, 'w') as jobscript:
            jobscript.write("""
universe                = vanilla
...
Executable              = {script}
Arguments               = {cmsswbase}
Output                  = {thisjobdir}/{sname}.out
Error                   = {thisjobdir}/{sname}.err
Log                     = {thisjobdir}/{sname}.log
use_x509userproxy       = true
initialdir              = {destdir}
Should_Transfer_Files   = YES
transfer_input_files    = {thisjobdir}/{datacard}
WhenToTransferOutput    = ON_EXIT
Queue
""".format(script=os.path.abspath('runlimits.sh'), sname=signal, thisjobdir=os.path.abspath(os.getcwd()), datacard=combinedDatacardTXT, cmsswbase=os.environ['CMSSW_BASE'], destdir=outputLocation))
        submit_command = 'condor_submit ' + os.path.abspath('submit_%s.cmd' % signal)
        os.chdir(currentDir)
        return submit_command


def runLimits(config, args):
    ...
    if os.path.exists(args.jobdir):
        print 'Removing old job dir: %s'%args.jobdir
        shutil.rmtree(args.jobdir)

    # move old output dir
    if os.path.exists(config.limitdir):
        t = time.localtime()
        moveStr = '_moveTime' + str(t.tm_year) + '-' + str(t.tm_mon) + '-' + str(t.tm_mday) + '-' + str(t.tm_hour * 10000 + t.tm_min * 100 + t.tm_sec)
        print 'renaming existing directory to', config.limitdir + moveStr
        os.rename(config.limitdir, config.limitdir + moveStr)


    if args.submittype == 'condor':
        pool = Pool(args.num_threads)
        submit_commands = pool.map(functools.partial(calcLimit, config, args), signals)
    ...
    elif args.submittype == 'interactive':
        # run limits in parallel
        print 'Running limits for %d signal points. Please wait...'%len(config.signals)
        pool = Pool(args.num_threads)
        results = pool.map(functools.partial(calcLimit, config, args), signals)
