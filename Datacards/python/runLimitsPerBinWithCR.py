...
class LimitConfig:
  ...
  def runLimits(self):
    ...
    for signal in self.signals: 
      ...
      srCards={} # srbin->[crbins]
      for dc in datacards:
          if 'lepcr' in dc or 'phocr' in dc or 'qcdcr' in dc: continue
          srCards[dc] = []
          rateParams = []
#           print '>>>', dc
          with open(os.path.join(datacardSaveLocation,dc)) as f:
              for l in f:
                  if 'rateParam' not in l: continue
                  pars = l.split()
#                   print '>>>>>>', pars
                  if 'R_' in pars[-1]:
                      rateParams += pars[-1].replace(' ','').split(',')
                  elif 'cr' in pars[0]:
                      rateParams += [pars[0]]
                  else:
...
#           print '>>>', rateParams
          for r in rateParams:
              srCards[dc].append(r.replace('R_','')+'.txt')
#           print '>>>', srCards[dc]
      ...
      # === Asymptotic
      if self.limitmethod=='AsymptoticLimits':
        for datacard in srCards:

          crcards = srCards[datacard]
          
          combineDatacardsCommand = 'combineCards.py ' + os.path.join(datacardSaveLocation,datacard)
          for cr in crcards:
              combineDatacardsCommand += ' ' + os.path.join(datacardSaveLocation,cr)
          combineDatacardsCommand += ' > ' + combinedDatacard
