...
def fillSignificances(config, sigfile, name):
    ...
    if 'T2fbd' in sigfile or 'T2bWL' in sigfile or 'T2cc' in sigfile:
#         mlsp_step = 10
        minmlsp = 0
        maxmlsp = 85
...
def calcLimit(config, args, signal):
    ...
    if config.limitmethod == 'AsymptoticLimits':
        ...
        runLimitsCommand = 'combine -M AsymptoticLimits -H ProfileLikelihood ' + combinedDatacard + ' -n ' + signal
        if (mstop<450 and 'fbd' not in sigtype) or (mstop >= 350 and mlsp < 350 and 'T2tt' in sigtype) :
            runLimitsCommand = 'combine -M AsymptoticLimits -H ProfileLikelihood ' + combinedDatacard + ' --rMin 0 --rMax 10 -n ' + signal
        if ('fbd' in sigtype or '4bd' in sigtype) and (mstop<=250):
            runLimitsCommand = 'combine -M AsymptoticLimits -H ProfileLikelihood ' + combinedDatacard + ' --rMin 0 --rMax 1 -n ' + signal
