...
from array import *
from sms import *
from smsPlotABS_Cut_v2 import *

# class producing the 2D plot with xsec colors
class smsPlotXSEC_CUT_v2(smsPlotABS):
    ...
    def __init__(self, modelname, histo, histo_cut, obsLimits, expLimits, expLimits_Cut, energy, lumi, preliminary, label):
        ...
        self.histo_cut = histo_cut['histogram_cut']
        # canvas style
        self.setStyle()
        self.setStyleCOLZ()
    ...
    def DrawPaletteLabel(self, Method):
        if Method == 0: textCOLZ = rt.TLatex(0.98,0.15,"#frac{#sigma_{cut} - #sigma}{#sigma}")
        if Method == 1: textCOLZ = rt.TLatex(0.98,0.15,"#sigma_{cut} - #sigma [pb]")
        if Method == 2: textCOLZ = rt.TLatex(0.98,0.15,"#sigma_{cut}/#sigma")
        if Method == 3: textCOLZ = rt.TLatex(0.98,0.15,"#frac{#sigma_{cut} - #sigma}{#sqrt{#sigma}} [#sqrt{pb}]")
        textCOLZ.SetNDC()
