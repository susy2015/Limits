...
class smsPlotABS(object):
    ...
    def __init__(self, modelname, histo, histo_cut, obsLimits, expLimits, expLimits_cut, energy, lumi, preliminary, label):
        ...
        self.histo = histo
	self.histo_cut = histo_cut

    def standardDef(self, modelname, histo, histo_cut, obsLimits, expLimits, expLimits_cut, energy, lumi, preliminary):
        # which SMS?
        self.model = sms(modelname)
        self.OBS = obsLimits
        self.EXP = expLimits
        self.EXP_CUT = expLimits_cut
        self.lumi = lumi
        self.energy = energy
	...
	textDphiCut = rt.TLatex(0.15,0.825,"Cut Comparison #Delta#phi(t^{sr}_{1,2}, #slash{#it{E}}_{T}) > 0.7")
	textDphiCut.SetNDC()
	textDphiCut.SetTextAlign(13)
	textDphiCut.SetTextFont(42)
	textDphiCut.SetTextSize(0.035)
	textDphiCut.Draw()
	self.c.textDphiCut = textDphiCut
    ...
    def DrawLegend(self):
        ...
        if self.model.extraText :
            textObs = rt.TLatex(self.model.Xmin+11*xRange/100, self.model.Ymax-1.65*yRange/100*10, "Expected w/ #Delta#phi(t^{sr}_{1,2}, #slash{#it{E}}_{T}) > 0.7 #pm 1 #sigma")
        else :
            textObs = rt.TLatex(self.model.Xmin+11*xRange/100, self.model.Ymax-1.50*yRange/100*10, "Expected w/ #Delta#phi(t^{sr}_{1,2}, #slash{#it{E}}_{T}) > 0.7 #pm 1 #sigma")
        ...
        if self.model.extraText :
            textExtra1 = rt.TLatex(self.model.Xmin+65*xRange/100, self.model.Ymax-1.05*yRange/100*10, self.model.extratext1)
        ...
        if self.model.extraText :
            textExp = rt.TLatex(self.model.Xmin+11*xRange/100, self.model.Ymax-2.30*yRange/100*10, "Expected #pm 1 #sigma")
        else :
            textExp = rt.TLatex(self.model.Xmin+11*xRange/100, self.model.Ymax-2.15*yRange/100*10, "Expected #pm 1 #sigma")
    ...
    def DrawLines(self):
        ...
        for exp in self.EXP['minus'] :
            ...
            exp.SetLineStyle(7)
            exp.SetLineWidth(2)                        
        for exp in self.EXP_CUT['plus'] :
            exp.SetLineColor(color(self.EXP_CUT['colorLine']))
            exp.SetLineStyle(7)
            exp.SetLineWidth(2)                
        # expected
        for exp in self.EXP_CUT['nominal'] :
            exp.SetLineColor(color(self.EXP_CUT['colorLine']))
            exp.SetLineStyle(7)
            exp.SetLineWidth(4)        
        # expected - 1sigma
        for exp in self.EXP_CUT['minus'] :
            exp.SetLineColor(color(self.EXP_CUT['colorLine']))
            exp.SetLineStyle(7)
            exp.SetLineWidth(2)                        
        # DRAW LINES
