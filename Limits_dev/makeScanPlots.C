...
void makeScanPlots(const TString inputFileName = "results_T2tt.root", const TString outputFileName = "limit_scan_T2tt.root", bool expectedOnly = false, bool additionalInterpolation = false)
{
  ...
  bool isCut = true;
  if(isCut){
    hlimexp->Write("hXsec_exp_corr_cut");
    hlimobs->Write("hXsec_obs_corr_cut");
    hxsecexp->Write("hXsec_exp_cut");
    hxsecobs->Write("hXsec_obs_cut");
    ...
    for(unsigned int ilim = 0; ilim < cexp.size(); ++ilim) {
      TString add = ilim > 0 ? "_" + TString(to_string(ilim)) : "";
      cexp.at(ilim)->SetName(gname + "_Exp_cut" + add);
      cexp.at(ilim)->Write(gname + "_Exp_cut" + add);
    }
    for(unsigned int ilim = 0; ilim < cexpup.size(); ++ilim) {
      TString add = ilim > 0 ? "_" + TString(to_string(ilim)) : "";
      cexpup.at(ilim)->SetName(gname + "_ExpP_cut" + add);
      cexpup.at(ilim)->Write(gname + "_ExpP_cut" + add);
    }
    for(unsigned int ilim = 0; ilim < cexpdown.size(); ++ilim) {
      TString add = ilim > 0 ? "_" + TString(to_string(ilim)) : "";
      cexpdown.at(ilim)->SetName(gname + "_ExpM_cut" + add);
      cexpdown.at(ilim)->Write(gname + "_ExpM_cut" + add);
    }
  }
  else{
